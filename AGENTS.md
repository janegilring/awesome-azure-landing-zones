...
| `terraform` | stdio (Docker, or pre-compiled binary) | Terraform Registry — providers, modules (incl. **AVM for Terraform**), policies. Docker-free option: point the server at the [pre-compiled binary](https://developer.hashicorp.com/terraform/mcp-server/deploy/local#pre-compiled-binary) (see [`.vscode/mcp.json`](.vscode/mcp.json)). | `alz-accelerator-expert`, `landing-zone-vending`. |
